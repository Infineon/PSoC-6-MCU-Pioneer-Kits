...
* Version: 1.10
*
* Description: This file contains the functions that initialize the EINK display
...
*******************************************************************************/
/******************************************************************************
* This file contains the functions that initialize the EINK display and show the
...
/* Flag that indicates if the EINK display has been detected */
...
********************************************************************************
* Summary:
*  Initializes the EINK display hardware and clears the display
*
* Parameters:
*  None
*
...
{ 
    /* Initialize the EINK display hardware with the given ambient temperature */
    ...
    if ( Cy_EINK_Power(CY_EINK_ON)== CY_EINK_OPERATION_SUCCESS)
    {
        ...
        /* Set the flag that indicates if the EINK display has been detected */
...
*  writes the frame buffer to the EINK display to show the instructions to use
*  this code example
*
* Parameters:
*  None
...
void DisplayInstructions(void)
{
    ...
    uint8_t const imageCoordinates[]  = IMAGE_COORDINATES;
    uint8_t const textOrigin[]        = TEXT_ORIGIN;
    
    /* Do this only if the EINK display has been detected */
    if (displayDetected)
    {
        /* Load the image that has the logo and the heading to the frame buffer */
        Cy_EINK_ImageToFrameBuffer(frameBuffer, (cy_eink_image_t*)logoAndHeading,
        ...
        /* Write the frame buffer to the EINK display. Automatic power control is
